...
double distance(Point const &p0, Point const &p1) { return (p1 - p0).norm(); }
...
double sign(Point &p1, Point &p2, Point &p3) { return sign(&p1, &p2, &p3); }
...
#define IS_WALL(p) p->get_type() == EntityType::Wall
  ...
  if (IS_SPHERIC(p1) && IS_SPHERIC(p2)) {
    return geometric::distance(*reinterpret_cast<SphericParticle *>(p1),
                               *reinterpret_cast<SphericParticle *>(p2));
  }

  if (IS_SPHERIC(p1) && IS_WALL(p2)) {
    return geometric::distance(*reinterpret_cast<SphericParticle *>(p1),
                               *reinterpret_cast<Wall *>(p2));
  }

  if (IS_WALL(p1) && IS_SPHERIC(p2)) {
    return geometric::distance(*reinterpret_cast<SphericParticle *>(p2),
                               *reinterpret_cast<Wall *>(p1));
  }
