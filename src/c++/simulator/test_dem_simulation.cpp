...
TEST_CASE("Test Particle Collision")
{
	...
	double final_time = 0.0;
 	integrate(domain, 0.001, 0.0, 0.02, [&](double time, std::vector<std::shared_ptr<IParticle>> const& particles)
	{
...
TEST_CASE("Test Particle to Wall Collision")
{
	...
	/* Simulate */
	double final_time = 0.0;
	integrate(domain, 0.001, 0.0, 0.02, [&](double time, std::vector<std::shared_ptr<IParticle>> particles){
		if (std::abs(time - 0.001) < 1e-8) {
