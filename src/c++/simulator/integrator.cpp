...
void integrate(
    ...
    std::function<void(double, std::vector<std::shared_ptr<Particle>> const
                                   &)> const &callback) {
  domain.set_time(initial_time);

  while (domain.get_time() < final_time) {
    domain.interaction_force_assembler->search_contacts(domain.particles, domain.boundaries);
