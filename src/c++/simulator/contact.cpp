...
ContactPair::ContactPair(std::shared_ptr<SphericParticle> i, std::shared_ptr<SphericParticle> j) 
{
...
ContactBoundary::ContactBoundary(std::shared_ptr<SphericParticle> p, std::shared_ptr<Boundary> b)
{
...
void ParticleCollection::add_contact_pair(std::shared_ptr<ContactBoundary> pair)
{
	particle_sets.push_back(pair);
}
